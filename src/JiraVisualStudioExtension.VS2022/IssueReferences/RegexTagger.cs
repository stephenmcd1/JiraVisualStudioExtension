...
namespace JiraVisualStudioExtension.IssueReferences
{
    ...
    internal abstract class RegexTagger<T, TMatchMetadata> : ITagger<T>
        where T : ITag
    {
        private readonly ITextBuffer _buffer;
        private readonly IClassifier _classifier;
        private readonly List<(Regex Expression, TMatchMetadata Metadata)> _matchExpressions;

        public RegexTagger(ITextBuffer buffer, IEnumerable<(Regex Expression, TMatchMetadata Metadata)> matchExpressions,
            IClassifier classifier)
        {
            _matchExpressions = matchExpressions.ToList();
            if (_matchExpressions.Any(re => (re.Expression.Options & RegexOptions.Multiline) == RegexOptions.Multiline))
            {
        ...
        public virtual IEnumerable<ITagSpan<T>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            foreach (var line in GetIntersectingLines(spans))
            {
                ...
                {
                    foreach (var match in regex.Expression.Matches(text).Cast<Match>())
                    {
                        foreach (var (tag, index, length) in TryCreateTagsForMatch(match, regex.Metadata))
                        {
                            if (tag != null)
                            {
        ...
        protected abstract IEnumerable<(T, int LineIndex, int SubmatchLength)> TryCreateTagsForMatch(Match match, TMatchMetadata metadata);
